            ...
            <div class="card-body hover-scroll-overlay-y">
                <form class="form-data" enctype="multipart/form-data">
                    ...
                    <div class="mb-10">
                        ...
                        <select name="fakultas" class="form-select" data-control="select2" id="from_select"
                    ...
                    <div class="mb-10">
                        ...
                        <input type="number" id="jumlah_l" class="form-control" name="jumlah_l">
                        <small class="text-danger jumlah_l_error"></small>
                    </div>

                    <div class="mb-10">
                        <label class="form-label">Jumlah Perempuan</label>
                        <input type="number" id="jumlah_p" class="form-control" name="jumlah_p">
                        <small class="text-danger jumlah_p_error"></small>
                    </div>
...
@section('script')
    <script>
        ...
        $(document).on('click', '#button-side-form', function() {
            control.overlay_form('Tambah', 'Data Maba');
        });
        ...
        $(document).on('submit', ".form-data", function(e) {
            ...
            if (type == 'add') {
                control.submitFormMultipartData('/admin/add-data-maba', 'Tambah',
                    'Data Maba',
                    'POST');
            } else {
                let uuid = $("input[name='uuid']").val();
                control.submitFormMultipartData('/admin/update-data-maba/' + uuid,
                    'Update',
                    'Data Maba', 'POST');
            }
        });
        ...
        $(document).on('click', '.button-update', function(e) {
            e.preventDefault();
            let url = '/admin/show-data-maba/' + $(this).attr('data-uuid');
            control.overlay_form('Update', 'Data Maba', url);
        })

        $(document).on('click', '.button-delete', function(e) {
            e.preventDefault();
            let url = '/admin/delete-data-maba/' + $(this).attr('data-uuid');
        ...
        const initDatatable = async () => {
            ...
            $('#kt_table_data').DataTable({
                ...
                columnDefs: [{
                    ...
                    render: function(data, type, full, meta) {
                        return `
                            ...
                            <a href="javascript:;" type="button" data-uuid="${data}" data-label="Data Maba" class="btn btn-danger button-delete btn-icon btn-sm">
                                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                rowGroup: {
                    dataSrc: 'tahun_masuk', // Ganti 'jenis_fasilitas_kesehatan' dengan nama kolom yang ingin digunakan sebagai grup
                        ...
                        if (!groupedData[group]) {
                            ...
                            return $("<tr/>")
                                .append(
                                    ...
                                    '<span style="font-weight: bold; color: #5caceb; padding: 2px 12px; font-size: 12px; border-radius: 8px; text-transform: uppercase;"> Angkatan = ' +
                                    group +
                                    '</span>' +
                                    '</td>'
        ...
        const fakultas = [{
            ...
            text: "Fakultas Dakwah dan Komunikasi"
        }, {
            text: "Fakultas Syariah dan Hukum"
        }, {
            text: "Fakultas Kedokteran dan Ilmu Kesehatan"
        }, {
            text: "Fakultas Adab dan Humaniora"
        }, {
            text: "Fakultas Ushuluddin dan Filsafat"
        }, {
            text: "Fakultas Ekonomi dan Bisnis Islam"
        }, ];

        $(function() {
            control.push_select3(fakultas, '#from_select');
            initDatatable();
        });
    </script>
@endsection
